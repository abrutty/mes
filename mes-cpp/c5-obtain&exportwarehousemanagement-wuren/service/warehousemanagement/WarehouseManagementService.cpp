...
WarehouseManagementPageDTO::Wrapper WarehouseManagementService::listAll(const WarehouseManagementQuery::Wrapper& query)
{
	...
	for (WarehouseManagementDO sub : result)
	{
		...
		// 		dto->sex = sub.getSex();
		// 		dto->age = sub.getAge();
		ZO_STAR_DOMAIN_DTO_TO_DO(data, dto, Item_Code, item_code, Item_Name, item_name, Batch_Code, batch_code)
		ZO_STAR_DOMAIN_DTO_TO_DO(data, dto, Warehouse_Name, warehouse_name, Vendor_Code, vendor_code, Vendor_Name, vendor_name)
		pages->addData(dto);
	}
	return pages;
}